...
class MpcCalibrate:
    ...
    # K1 Max Compatibility Polyfill
    def _wait_while(self, callback, error_on_cancel=False, delay=1.0):
        reactor = self.printer.get_reactor()
        eventtime = reactor.monotonic()
        while not self.printer.is_shutdown():
            if not callback(eventtime):
                return
            eventtime = reactor.pause(eventtime + delay)
    ...
    def wait_stable(self, cycles=5):
        ...
        self._wait_while(process, True, 0.2)
    ...
    def measure_power(self, max_time, sample_time):
        ...
        def process(eventtime):
            ...
            # FIX: Use 'power' directly if 'control_stats' is missing
            if "control_stats" in status:
                pwr = status["control_stats"]["power"]
            else:
                pwr = status["power"]
            
            samples.append((dt, pwr * dt))
            time[0] += dt
